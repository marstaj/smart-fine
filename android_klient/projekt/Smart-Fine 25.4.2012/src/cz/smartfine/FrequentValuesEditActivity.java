...
public class FrequentValuesEditActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		switch (valuesGroup) {
			case 1 :
				...
				fav_list = app.getFrequentValuesDAO().getFav_mpzValues();
				break;
			case 2 :
				list = app.getFrequentValuesDAO().getVehicleBrandValues();
				fav_list = app.getFrequentValuesDAO().getFav_vehicleBrandValues();		
				break;
		}
	...
	protected void onDestroy() {
		try {
			app.getFrequentValuesDAO().saveFrequentValues();
		} catch (Exception e) {
			Toaster.toast("Chyba pøi ukládání seznamu nejèastejších hodnot.", Toaster.LONG);
		}
		super.onDestroy();
	}
