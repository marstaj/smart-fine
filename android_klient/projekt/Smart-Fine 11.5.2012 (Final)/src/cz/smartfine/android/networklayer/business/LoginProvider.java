...
public class LoginProvider implements ILoginProtocolListener {
	...
	public static int getPIN(Context context) {
		// získá přístup k nastavení aplikace a poté získá hodnotu služebního
		// čísla//
		return PreferenceManager.getDefaultSharedPreferences(context).getInt(LoginProvider.PREF_PIN_KEY_NAME, -1);
	}
	...
	private void saveLoginInformation(Context context, int badgeNumber, int pin) {
		// vloží do nastavení služební číslo a PIN//
		PreferenceManager.getDefaultSharedPreferences(context).edit().putInt(LoginProvider.PREF_BADGENUMBER_KEY_NAME, badgeNumber).commit();
		PreferenceManager.getDefaultSharedPreferences(context).edit().putInt(LoginProvider.PREF_PIN_KEY_NAME, pin).commit();
	}

	private void deleteLoginInformation(Context context) {
		// odebere služební číslo a PIN z nastavení//
		PreferenceManager.getDefaultSharedPreferences(context).edit().remove(LoginProvider.PREF_BADGENUMBER_KEY_NAME).commit();
		PreferenceManager.getDefaultSharedPreferences(context).edit().remove(LoginProvider.PREF_PIN_KEY_NAME).commit();
	}
}