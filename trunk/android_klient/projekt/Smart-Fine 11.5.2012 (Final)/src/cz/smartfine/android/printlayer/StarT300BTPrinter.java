...
public class StarT300BTPrinter implements IPrinter {
	...
	public void printTicket(Ticket ticket) throws StarIOPortException {
		// TODO Napsat popisky do R a javadoc!!!
		...
		row += 80;
		c.drawText("VÝZVA", 288, row, title); // Title
		row += 50;
		c.drawText("pro osobu podezřelou ze spáchání přestupku", 288, row, subtitle); // Subtitle

		row += 70;
		c.drawText("Datum: ", 20, row, section); // Datum
		...
		row += 30;
		c.drawText("Čas: ", 20, row, section); // Cas
		...
		row += 30;
		c.drawText("Odtah: ", 20, row, section); // Odtah
		if (ticket.isTow()) {
			data = "ANO";
		} else {
			data = "NE";
		...
		c.drawText("Sb. ", 370, row, section); // sbirky

		row += 30;
		c.drawText("Dostavte se nejpozdeji do 5 dnů na služebnu městské ", 20, row, tiny); // dostavit se na sluzebnu
		row += 30;
		c.drawText("policie, uvedenou v této výzvě.", 20, row, tiny); // dostavit se na sluzebnu

		row += 210;
		c.drawText("Razítko", 144, row, down); // razitko
		c.drawText("Podpis strážníka", 432, row, down); // podpis straznika
