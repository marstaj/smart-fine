...
public class TicketListActivity extends Activity {
	...
	public void uploadToServerClick(View button) {
		...
		builder.setMessage(getText(R.string.view_alert_upload));
	...
	private void uploadWaypoints() {
		...
		if (!waypoints.isEmpty()) {
			...
			if (Phone.isConnectedToInternet(getApplicationContext())) {
				...
				progDialog.setTitle(getText(R.string.view_dialog_upload).toString());
				progDialog.setMessage(getText(R.string.please_wait).toString());
				...
				Toaster.toast(R.string.internet_need_upload, Toaster.SHORT);
			}
		} else {
			uploadTickets();
		}
	...
	public void uploadTickets() {
		...
		if (Phone.isConnectedToInternet(this)) {
			...
			// Nastavení dialogu a LogIN
			progDialog = new ProgressDialog(this);
			progDialog.setTitle(getText(R.string.view_dialog_upload));
