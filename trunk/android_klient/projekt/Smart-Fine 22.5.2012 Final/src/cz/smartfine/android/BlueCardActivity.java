	...
	public void checkCardClick(View button) {
		...
		if (Phone.isConnectedToInternet(getApplicationContext())) {
			...
			if (helper.getSpcNumber().length() == 10) {
				...
				progDialog.setTitle(getText(R.string.view_dialog_card));
				...
				Toaster.toast(R.string.val_card_err, Toaster.SHORT);
			}
		} else {
			Toaster.toast(R.string.internet_need_card, Toaster.SHORT);
		}
	}
	...
	public void setInfo(Serializable infoFromServer) {
		...
		if (status != null) {
			if (status == SPCStatus.OK_SPC) {
				((TextView) findViewById(R.id.blueCardStatus)).setText(getText(R.string.view_card_ok));
			}
			if (status == SPCStatus.STOLEN_SPC) {
				((TextView) findViewById(R.id.blueCardStatus)).setText(getText(R.string.view_card_stolen));
			}
			if (status == SPCStatus.UKNOWN_SPC_STATUS) {
				((TextView) findViewById(R.id.blueCardStatus)).setText(getText(R.string.view_card_unknown));
			}
		}
