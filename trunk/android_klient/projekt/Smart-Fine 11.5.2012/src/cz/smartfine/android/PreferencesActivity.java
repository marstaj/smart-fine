	...
	public boolean onPreferenceChange(Preference preference, Object newValue) {
		...
		if (key.equals("pref_syncserverurl")) {
			...
			if (StringValidator.isURLValid(value)) {
				...
				new AlertDialog.Builder(this).setTitle("Chyba").setMessage("Špatný formát adresy").setNeutralButton("Zavřít", null).show();
				return false;
			}
		}
		...
		if (key.equals("pref_btprinter_macaddress")) {
			...
			if (StringValidator.isMACValid((String) newValue)) {
				...
				//TODO: vlo�it text chyby do xml
				new AlertDialog.Builder(this).setTitle("Chyba").setMessage("spatny format mac").setNeutralButton("Zavřít", null).show();
				return false;
			}
		}
		return true;
