...
public class SMSParkingProtocol implements IDataProtocol {
	 ...
	 * @param smsParkingProtocolListener
	 *            Posluchač událostí z této třídy.
	 */
	public SMSParkingProtocol(INetworkInterface networkInterface,
			ISMSParkingProtocolListener smsParkingProtocolListener) {
	...
	/**
	 * Odebere posluchače událostí protokolu pro kontrolu času parkování
	 * vozidel.
	 * 
	 * @param smsParkingProtocolListener
	 *            Posluchač událostí protokolu pro kontrolu parkování.
	 */
	public void removeSMSParkingProtocolListener(
			ISMSParkingProtocolListener smsParkingProtocolListener) {
	 ...
	 *            Posluchač událostí protokolu pro kontrolu parkování.
	 */
	public void setSMSParkingProtocolListener(
			ISMSParkingProtocolListener smsParkingProtocolListener) {
		this.smsParkingProtocolListener = smsParkingProtocolListener;
	}
	...
	public void onReceivedData(byte[] receivedData) {
		...
		if (smsParkingProtocolListener != null) {
			...
			case MobileMessageIDs.ID_MSG_STATUS_PSMS: // úspěšné přihlášení//
				...
				parkingSince = new Date(Conventer.byteArrayToLong(receivedData,
						2)); // načtení času od kdy smí vozidlo parkovat
				parkingUntil = new Date(Conventer.byteArrayToLong(receivedData,
						10)); // načtení času do kdy smí vozidlo parkovat

				int rvpLength = Conventer.byteArrayToInt(receivedData, 18); // zjištění
																			// délky
																			// pole
																			// s
																			// SPZ
				vehicleRegistrationPlate = new String(receivedData, 22,
						rvpLength); // převedení pole bytů na string

				smsParkingProtocolListener
						.onReceivedSMSParkingInfo(new SMSParkingInfo(
								parkingStatus, parkingSince, parkingUntil,
	 ...
	 *            SPZ vozidla, u kterého je požadováno zjištění stavu parkování.
	 */
	public void checkParking(String vehicleRegistrationPlate) {
		if (networkInterface != null) {
			networkInterface.sendData(
					createPSMSMessage(vehicleRegistrationPlate), this);
		}
	}
	...
	protected byte[] createPSMSMessage(String vehicleRegistrationPlate) {
		...
		msg.putArrayWithIntLength(vehicleRegistrationPlate.getBytes()); // vloží
																		// délku
																		// pole
																		// a
																		// pole
