...
public class LoginProvider implements ILoginProtocolListener {
	...
	/**
	 * Název klíče v preferencích, pro přístup ke služebnímu číslu přihlášeného
	 * policisty.
	 */
	 ...
	 *            Síťové rozhraní pro komunikaci datového protokolu.
	 * @param appContext
	 *            Kontext aplikace.
	 */
	 ...
	 * @param appContext
	 *            Kontext aplikace.
	 * @param loginProviderListener
	 *            Posluchač událostí LoginProideru.
	 */
	public LoginProvider(INetworkInterface networkInterface,
			Context appContext, ILoginProviderListener loginProviderListener) {
		this.networkInterface = networkInterface;
		this.appContext = appContext;
	 ...
	 *            Posluchač událostí LoginProideru.
	 */
	public void setLoginProviderListener(
			ILoginProviderListener loginProviderListener) {
		this.loginProviderListener = loginProviderListener;
	}
	 ...
	 * 
	 * @param badgeNumber
	 *            Služební číslo přihlašovaného policisty.
	 * @param pin
	 *            PIN přihlašovaného policisty.
	 * @param imei
	 *            Identifikační číslo mobilního zařízení (IMEI), ze kterého se
	 *            přihlašuje.
	 */
	public void login(int badgeNumber, int pin, String imei) {
		...
		// nastavení vnitřních proměnných pro následné zapamatování služebního
		// čísla a pinu//
		this.lastBadgeNumber = badgeNumber;
		this.lastPin = pin;

		this.loginProtocol.loginToServer(badgeNumber, pin, imei); // odeslání
																	// přihlašovací
																	// zprávy
	}
	...
	public void logout() {
		...
		logoutInProgress = true; // nastavení stavové proměnné určující, že
									// probíhá odhlašování
		this.loginProtocol.logoutFromServer(); // poslání odhlašovací zprávy
		deleteLoginInformation(this.appContext); // odstranění přihlašovacích
													// údajů z preferencí
	}
	 ...
	 * 
	 * @param appContext
	 *            Kontext aplikace pro přístup k preferencím.
	 * @return true, přihlašovací údaje jsou k dispozici, false přihlašovací
	...
	public static boolean isAvaibleLoginInformation(Context appContext) {
		...
		int bn = PreferenceManager.getDefaultSharedPreferences(appContext)
				.getInt(LoginProvider.PREF_BADGENUMBER_KEY_NAME, -1);
		int pin = PreferenceManager.getDefaultSharedPreferences(appContext)
				.getInt(LoginProvider.PREF_PIN_KEY_NAME, -1);
	...
	public static int getBadgeNumber(Context appContext) {
		// získá přístup k nastavení aplikace a poté získá hodnotu služebního
		// čísla//
	...
	public static String getIMEI(Context appContext) {
		...
		// zjistí, jestli bylo nějaké číslo vráceno a případně vrátí nulové
		// IMEI//
		if (imei != null) {
			return imei;
		} else {
	...
	public void onLoginConfirmed() {
		// po potvrzení přihlášení ze serveru, je možné přihlašovací údaje
		// uložit//
		if (this.lastBadgeNumber != -1 && this.lastPin != -1) {
			saveLoginInformation(this.appContext, this.lastBadgeNumber,
					this.lastPin); // uložení přihlašovacích údajů
	...
	private void saveLoginInformation(Context appContext, int badgeNumber,
			int pin) {
		// vloží do nastavení služební číslo a PIN//
		PreferenceManager.getDefaultSharedPreferences(appContext).edit()
				.putInt(LoginProvider.PREF_BADGENUMBER_KEY_NAME, badgeNumber)
				.commit();
		PreferenceManager.getDefaultSharedPreferences(appContext).edit()
				.putInt(LoginProvider.PREF_PIN_KEY_NAME, pin).commit();
	}

	private void deleteLoginInformation(Context appContext) {
		// odebere služební číslo a PIN z nastavení//
		PreferenceManager.getDefaultSharedPreferences(appContext).edit()
				.remove(LoginProvider.PREF_BADGENUMBER_KEY_NAME).commit();
		PreferenceManager.getDefaultSharedPreferences(appContext).edit()
				.remove(LoginProvider.PREF_PIN_KEY_NAME).commit();
	}
}