...
) ENGINE=InnoDB DEFAULT CHARSET=cp1250 COMMENT='Tabulka asociací mezi policisty a mobilními zaøízeními.';
...
CREATE TABLE `mobiledevices` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `IMEI` char(15) NOT NULL COMMENT 'Èíslo IMEI mobilního zaøízení.',
  `NAME` varchar(20) NOT NULL COMMENT 'Název mobilního zaøízení',
  `DESCRIPTION` varchar(255) NOT NULL DEFAULT '' COMMENT 'Popis mobilního zaøízení',
  `OFFICE` int(10) unsigned NOT NULL COMMENT 'Služebna, kde je zaøízení registrované.',
  `BLOCKED` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Urèuje, zda není zaøízení blokováno.',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `IMEI` (`IMEI`),
  KEY `FKMOBILEDEVICESOFFICE` (`OFFICE`)
) ENGINE=InnoDB DEFAULT CHARSET=cp1250 COMMENT='Tabulka mobilních zaøízení.';
...
CREATE TABLE `policemen` (
  ...
  `LASTNAME` varchar(20) NOT NULL COMMENT 'Pøíjmení.',
  `BADGENUMBER` int(11) NOT NULL COMMENT 'Služební èíslo.',
  `PIN` int(11) NOT NULL COMMENT 'PIN.',
  `PERMISSIONS` int(10) unsigned NOT NULL DEFAULT '0' COMMENT 'Oprávnìní.',
...
) ENGINE=InnoDB DEFAULT CHARSET=cp1250 COMMENT='Tabulka policistù.';
...
CREATE TABLE `spcblacklist` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SPCNUMBER` varchar(10) NOT NULL COMMENT 'Èíslo PPK.',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `SPCNUMBER` (`SPCNUMBER`)
) ENGINE=InnoDB DEFAULT CHARSET=cp1250 COMMENT='Black list pøenosných parkovacích karet.';
...
) ENGINE=InnoDB DEFAULT CHARSET=cp1250 COMMENT='Tabulka fotografií k parkovacím lístkùm.';
